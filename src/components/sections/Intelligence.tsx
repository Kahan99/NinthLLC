...
export default function Intelligence() {
  ...
  // Vertical infinite-feeling sliding offset mapping to scroll
  const yTranslate = useTransform(smoothProgress, [0, 1], ["0%", "-60%"]);

  // Right-side features fade in sequentially
  const f1Opacity = useTransform(smoothProgress, [0.1, 0.3], [0.3, 1]);
  const f2Opacity = useTransform(smoothProgress, [0.4, 0.6], [0.3, 1]);
  const f3Opacity = useTransform(smoothProgress, [0.7, 0.9], [0.3, 1]);
  
  const f1Scale = useTransform(smoothProgress, [0.1, 0.3], [0.98, 1]);
  const f2Scale = useTransform(smoothProgress, [0.4, 0.6], [0.98, 1]);
  const f3Scale = useTransform(smoothProgress, [0.7, 0.9], [0.98, 1]);

  // Expand the logo array to guarantee it can scroll sufficiently
  const displayLogos = [...aiLogos, ...aiLogos, ...aiLogos, ...aiLogos];

  return (
    <div ref={containerRef} className="relative w-full xl:h-[250vh]">
      <div className="xl:sticky xl:top-0 xl:h-screen w-full flex flex-col justify-center bg-black">
        <div className="py-12 md:py-20 lg:py-24 w-full">
          <section className="flex flex-col gap-12 md:gap-20 items-stretch max-w-7xl mx-auto px-5 lg:px-0" id="intelligence">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-start">
                ...
                <div className="flex w-full items-center justify-start py-10">
                  <div className="relative w-full max-w-lg">
                    ...
                    <div className="rounded-3xl p-1 overflow-hidden shadow-[inset_0_0_8px_rgba(0,0,0,0.4),0_0_60px_rgba(0,0,0,0.2),0_30px_80px_rgba(0,0,0,0.5)]" style={{ backgroundImage: "radial-gradient(circle farthest-side at 50% 0, rgba(242, 242, 242, 0.2), transparent)", outline: "1px solid rgba(242, 242, 242, 0.15)", outlineOffset: "-1px" }}>
                      ...
                      <div className="rounded-2xl overflow-hidden border border-[rgba(242,242,242,0.3)] shadow-[inset_0_0_10px_rgba(0,0,0,0.1),0_0_12px_rgba(0,0,0,0.4)] backdrop-blur-md">
                        <div>
                          <div className="flex w-full relative h-80 rounded-2xl overflow-hidden bg-[#0a0a0a]">
                            
                            {/* Left Side: Scrolling AI Logos */}
                            <div className="w-1/2 relative overflow-hidden">
                              ...
                              <motion.div className="relative z-10 border-r border-[#252525]/30 pt-[300px]" style={{ y: yTranslate }}>
                                {displayLogos.map((logo, idx) => (
                                  <div key={idx} className="relative aspect-square flex items-center justify-center p-6 border-b border-[#252525]/30 w-full">
                            ...
                            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30">
                              <div className="size-10 rounded-full bg-neutral-900 border border-[#FFA16C]/30 flex items-center justify-center relative overflow-hidden drop-shadow-2xl">
                                <svg className="absolute animate-pulse" fill="none" height="39" viewBox="0 0 39 39" width="39" style={{ color: "rgba(255, 255, 255, 0.49)" }}>
                                ...
                                <svg className="absolute animate-pulse" style={{ animationDelay: '0.4s' }} fill="none" height="39" viewBox="0 0 39 39" width="39" style={{ color: "rgba(255, 255, 255, 0.49)" }}>
                              ...
                              <div className="relative aspect-square flex items-center justify-center p-6 border-none w-full">
                                ...
                                <div className="size-28 md:size-32 rounded-full relative flex items-center justify-center transition-all duration-300 before:absolute before:inset-0 before:rounded-full before:border before:border-neutral-800">
                                  <div className="rounded-full flex items-center justify-center transition-all duration-400 relative size-24 bg-neutral-800 scale-100 opacity-100" style={{ boxShadow: "0px 24px 32px -12px rgba(0, 0, 0, 0.5), 0px 16px 24px -8px rgba(0, 0, 0, 0.5), 0px 8px 16px -4px rgba(0, 0, 0, 0.5), 0px 0px 0px 1px rgba(255, 255, 255, 0.1)" }}>
                                    <div className="relative overflow-hidden" style={{ width: "56.35px", height: "42.66px" }}>
                                      {[...Array(9)].map((_, i) => (
                                        <div key={i} className="absolute rounded-full" style={{ backgroundColor: "rgba(112, 112, 121, 0.8)", width: "100%", left: "0px", top: `${i * 11.11}%`, height: "10%" }}></div>
                                      ))}
                                    </div>
                                  </div>
                                </div>
              ...
              <div className="flex flex-col gap-8 justify-center">
                ...
                <div className="flex flex-col gap-3">
                  <motion.div style={{ opacity: f1Opacity, scale: f1Scale }} className="group flex items-center gap-5 p-5 bg-white/[0.02] border border-white/5 rounded-xl transition-all duration-300 hover:bg-white/[0.04] hover:border-white/10">
                    <div className="shrink-0 flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/5 group-hover:border-[#FFA16C]/20 group-hover:bg-[#FFA16C]/10 transition-all duration-300">
                      <div className="transition-transform duration-300 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                        <div className="w-5 h-5 text-[#FFA16C]" aria-hidden="true">
                          <svg fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="12" x2="12" y1="12" y2="6"></line>
                            <line x1="12" x2="16" y1="12" y2="12"></line>
                          </svg>
                        </div>
                      </div>
                    </div>
                  ...
                  <motion.div style={{ opacity: f2Opacity, scale: f2Scale }} className="group flex items-center gap-5 p-5 bg-white/[0.02] border border-white/5 rounded-xl transition-all duration-300 hover:bg-white/[0.04] hover:border-white/10">
                    <div className="shrink-0 flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/5 group-hover:border-[#FFA16C]/20 group-hover:bg-[#FFA16C]/10 transition-all duration-300">
                      <div className="transition-transform duration-300 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                        <div className="w-5 h-5 text-[#FFA16C]" aria-hidden="true">
                          <svg fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <motion.div style={{ opacity: f3Opacity, scale: f3Scale }} className="group flex items-center gap-5 p-5 bg-white/[0.02] border border-white/5 rounded-xl transition-all duration-300 hover:bg-white/[0.04] hover:border-white/10">
                    <div className="shrink-0 flex items-center justify-center w-10 h-10 rounded-lg bg-white/5 border border-white/5 group-hover:border-[#FFA16C]/20 group-hover:bg-[#FFA16C]/10 transition-all duration-300">
                      <div className="transition-transform duration-300 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                        <div className="w-5 h-5 text-[#FFA16C]" aria-hidden="true">
                          <svg fill="none" height="20" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z" fill="currentColor"></path>
                            <path d="M20 3v4"></path>
